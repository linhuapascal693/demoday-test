...
8. Classics: 推荐2-3本与"${concept}"知识点直接相关的经典书籍，必须满足：
   - 书籍内容要直接涉及"${concept}"这个具体知识点
   - 如果是编程概念，推荐讲解该概念的权威书籍或经典教程
   - 如果是技术框架，推荐该框架的官方文档或最佳实践书籍
   - 如果是理论知识，推荐该领域的经典教材或权威著作
   
   每本书包含以下字段：
   - title: 书名（包含书名号《》）
   - author: 作者
   - description: 书籍简介（1-2句话，说明这本书的主要内容）
   - reason: 推荐理由（必须具体说明这本书如何帮助理解"${concept}"，包含该知识点的讲解方式或章节内容）
   - difficulty: 难度等级（入门级/进阶级/专家级，根据书籍对"${concept}"的讲解深度判断）
   
   重要：不要推荐与"${concept}"无关的通用书籍，必须确保每本书都直接涉及该知识点！
